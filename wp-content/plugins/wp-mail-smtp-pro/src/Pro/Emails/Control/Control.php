	...
	public function get_controls( $keys_only = false ) {
		...
		$data = array(
			...
			'auto_updates'     => array(
				...
				'emails' => array(
					'dis_auto_updates_status'   => array(
						'label' => esc_html__( 'Status', 'wp-mail-smtp-pro' ),
			...
			'network_new_site' => array(
				...
				'emails' => array(
					'dis_new_site_new_site_added_to_network'                     => array(
						'label' => esc_html__( 'Added To Network', 'wp-mail-smtp-pro' ),
						'desc'  => esc_html__( 'When WordPress was installed, and when a site was added to a Multisite network. Sent to the side admin.', 'wp-mail-smtp-pro' ),
					),
			...
			// 'other'            => array(
			// 'title'  => esc_html__( 'Other', 'wp-mail-smtp-pro' ),
			// 'emails' => array(
			// 'dis_other_fatal_error' => array(
			// 'label' => esc_html__( 'Fatal Error Occurred', 'wp-mail-smtp-pro' ),
			// 'desc'  => esc_html__( 'A fatal error occurred in a plugin or theme and Recovery Mode is not active. Sent to the site admin or value of the RECOVERY_MODE_EMAIL constant.', 'wp-mail-smtp-pro' ) . '<br>' .
			// '<strong>' . esc_html__( 'Disabling this option will completely disable the whole WordPress 5.2+ Recovery Mode.', 'wp-mail-smtp-pro' ) . '</strong>',
			// ),
			// ),
			// ),
		);
